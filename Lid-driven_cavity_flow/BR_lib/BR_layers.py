...
class Affine_linear_mapping(torch.nn.Module):
    ...
    def forward(self, inputs, logdet=None, reverse=False):
        ...
        assert len(self.lb) == self.n_dim 
        x = inputs
        lb_ = torch.tensor(self.lb, device=x.device)
        hb_ = torch.tensor(self.hb, device=x.device)
        ...
        if logdet is not None:
            dlogdet = torch.sum( torch.log( 1.0/(hb_-lb_) ) ) * torch.ones_like(x[:,0:1])
            if reverse:
                dlogdet *= -1.0
            return x, logdet + dlogdet
