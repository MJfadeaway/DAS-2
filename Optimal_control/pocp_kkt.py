# -*- coding: utf-8 -*-
"""
Created on Mon Mar 21 10:11:22 2022
...
def pred_lam(NetLam, X):
    return NetLam(X)

# Loss function
def loss_func_kkt(NetY, NetP, NetU, NetLam, lenth, InSet, BdSet):
    InSet.y = pred_y(NetY, lenth, InSet.x)
    InSet.p = pred_p(NetP, lenth, InSet.x)
    InSet.u = pred_u(NetU, InSet.x)
    InSet.lam1 = pred_lam(NetLam, InSet.x)[:,0:1]
    InSet.lam2 = pred_lam(NetLam, InSet.x)[:,1:2]
    ...
    InSet.loss_ic = (torch.relu(data.ua-InSet.u)**2).mean() + \
                    (torch.relu(InSet.u-data.ub)**2).mean() + \
                    (torch.relu(-InSet.lam1)**2).mean() + \
                    (torch.relu(-InSet.lam2)**2).mean()
    InSet.loss_var = ((data.alpha*InSet.u + InSet.p - InSet.lam1 + InSet.lam2)**2).mean()
    InSet.loss_com = ((InSet.lam1*InSet.u)**2).mean() + \
                     ((InSet.lam2*(data.ub-InSet.u))**2).mean()
                       
    loss = InSet.loss_pde_y + InSet.loss_pde_p + InSet.loss_var + \
           InSet.loss_com + InSet.loss_ic
   
    #loss = InSet.loss_pde_y + InSet.loss_pde_p + InSet.loss_grad   
    return loss**0.5


# optimality-conditions-loss
def KKT_loss(NetY, NetP, NetU, NetLam, lenth, InSet, BdSet, loss_history, record=True):    
    # c is the step_size
    loss = loss_func_kkt(NetY, NetP, NetU, NetLam, lenth, InSet, BdSet)**2

    if record:
        loss_history.append([((InSet.loss_pde_y)**0.5).item(), \
                             ((InSet.loss_pde_p)**0.5).item(), \
                             ((InSet.loss_var)**0.5).item(), \
                             ((InSet.loss_com)**0.5).item(), \
                             ((InSet.loss_ic)**0.5).item(), \
                               loss.item()])
            
    print('pde_y_loss: %.3e, pde_p_loss: %.3e, var_loss: %.3e,  com_loss: %.3e,  ic_loss: %.3e, KKT_loss: %.3e' \
          %((InSet.loss_pde_y)**0.5, (InSet.loss_pde_p)**0.5, (InSet.loss_var)**0.5,\
            (InSet.loss_com)**0.5, (InSet.loss_ic)**0.5, loss))
    return loss_history

# ----------------------------------------------------------------------------------------------------
# Train neural network
def train_kkt(NetY, NetP, NetU, NetLam, lenth, InSet, BdSet,\
             Optim, epochs, epochs_i, loss_history): 
    print('Train Neural Network')
    
    # Record the optimal parameters
    loss = loss_func_kkt(NetY, NetP, NetU, NetLam, lenth, InSet, BdSet).data
    print('epoch_yp: %d, loss_yp: %.3e, time: %.2f'
          %(0, loss, 0.00))
    ...
    for it in range(int(epochs/epochs_i)):
        ...
        def closure():
            Optim.zero_grad()
            loss = loss_func_kkt(NetY, NetP, NetU, NetLam, lenth, InSet, BdSet)
            loss.backward()
            return loss
        Optim.step(closure)
        loss = loss_func_kkt(NetY, NetP, NetU, NetLam, lenth, InSet, BdSet).data
        ...
        print('pde_y_loss: %.3e, pde_p_loss: %.3e, var_loss: %.3e,  com_loss: %.3e, ic_loss: %.3e, KKT_loss: %.3e, time: %.2f' \
          %((InSet.loss_pde_y)**0.5, (InSet.loss_pde_p)**0.5, (InSet.loss_var)**0.5,\
            (InSet.loss_com)**0.5, (InSet.loss_ic)**0.5, loss, elapsed))
    return loss_history
...
parser.add_argument('--layers_u', type=int, default=[4,25,25,25,1],
                    help='network structure')
parser.add_argument('--layers_lam', type=int, default=[4,25,25,25,2],
                    help='network structure')
...
net_lam = Net(args.layers_lam, device).to(device)
loss_history = []
loss_history = \
    KKT_loss(net_y, net_p, net_u, net_lam, lenth, in_set, bd_set, loss_history)
'''

net_y = torch.load('./net_save/test3-pinn-ynet.pt')
net_u = torch.load('./net_save/test3-pinn-unet.pt')
net_p = torch.load('./net_save/test3-pinn-pnet.pt')
net_lam = torch.load('./net_save/test3-pinn-lamnet.pt')
loss_history = torch.load('./net_save/test3-pinn-loss.pt').tolist()
...
#%%  Train neural network
start_time = time.time()
loss_history = train_kkt(net_y, net_p, net_u, net_lam, lenth, in_set, bd_set,
...
fname1 = "./net_save/test3-pinn-ynet.pt"
fname2 = "./net_save/test3-pinn-unet.pt"
fname3 = "./net_save/test3-pinn-pnet.pt"
fname4 = "./net_save/test3-pinn-lamnet.pt"
fname5 = "./net_save/test3-pinn-loss.pt"

torch.save(net_y, fname1)
torch.save(net_u, fname2)
torch.save(net_p, fname3)
torch.save(net_lam, fname4)
...
#np.save('./net_save/y-pinn-kkt-(%.2f,%.1f,%.2f,%.2f).npy'%(mu1,mu2,ua,ub),y)
np.save('./net_save/u-pinn-kkt-(%.2f,%.1f,%.2f,%.2f).npy'%(mu1,mu2,ua,ub),u)
...
lam1 = pred_lam(net_lam, te_data).cpu().detach().numpy()[:,0].flatten()
lam2 = pred_lam(net_lam, te_data).cpu().detach().numpy()[:,1].flatten()
yd = data.yd(te_data).cpu().detach().numpy().flatten()

x0 = x0.flatten()
x1 = x1.flatten()

ax0 = ax[0].tricontourf(x0, x1, lam1, num_line, cmap='coolwarm')
...
ax1 = ax[1].tricontourf(x0, x1, lam2, num_line, alpha=1, cmap='coolwarm')
